...
const initialModels: ModelItem[] = [
  { id: "m1", name: "边缘违章检测", version: "demo-config", accuracy: "待评测", latency: "待实测", status: "idle", scene: "安全帽 / 区域入侵 / 动火" },
  { id: "m2", name: "隐蔽工程结构化验真", version: "demo-config", accuracy: "待评测", latency: "待实测", status: "idle", scene: "沟槽 / 管道 / 线缆预埋" },
  { id: "m3", name: "图物对齐进度核算", version: "demo-config", accuracy: "待评测", latency: "待实测", status: "idle", scene: "GIS / 设计图 / 实景比对" },
  { id: "m4", name: "弱网任务补偿调度", version: "demo-config", accuracy: "不适用", latency: "待实测", status: "idle", scene: "边缘缓存 / 断点续传 / 调度" },
];
...
export const ModelServicePage: React.FC = () => {
  ...
  const toggleModel = (id: string) => {
    ...
    setNotice(`${target?.name || "模型"}：仅切换前端演示状态，尚未连接真实模型服务。`);
  };
    ...
    <div className="space-y-5 page-enter">
      ...
      <div className="rounded-[28px] border border-sky-100 bg-gradient-to-r from-sky-700 via-blue-700 to-cyan-700 p-6 text-white shadow-[0_20px_80px_-32px_rgba(14,116,255,0.8)]">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          ...
          <button
            onClick={() =>
              setNotice("原型动作：未导出服务报表，也无真实运行数据可写。")
            }
          ...
          <div className="rounded-[28px] border border-slate-200 bg-white p-5 shadow-sm">
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
              ...
              <button
                onClick={() =>
                  setNotice(
                    `原型动作：${activeModel.name} 未切换真实版本工件，仅保留 demo-config 标签。`
                  )
                }
              ...
              <button
                onClick={() => setNotice(`${activeModel.name} 尚未连接评测 worker，未产生吞吐、时延或显存数据。`) }
                className="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-medium text-slate-700 transition-all hover:border-sky-200 hover:text-sky-700"
              >
                性能压测
              </button>
              <button
                onClick={() => setNotice(`${activeModel.name} 尚未连接边缘节点，本按钮仅用于交互原型。`) }
                className="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-medium text-slate-700 transition-all hover:border-sky-200 hover:text-sky-700"
              >
                下发边缘节点
              </button>
          ...
          <div className="flex items-center justify-end gap-3">
            ...
            <button
              onClick={() => {
                setDetailOpen(false);
                setNotice(
                  `原型动作：${activeModel.name} 未执行推理，也未写入任何缓存或后端结果。`
                );
              }}
